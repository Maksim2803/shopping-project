class MenuManager:
    ...
    def add_button_part1(self,button,category):
        if not category:
            return "category not selected"
        button=self.name_check(button,category)
        if button=="name is taken" or button=="name name unavailable":
            return button
        board_spisok=self.unpacking(self.board[category])
        ...
        return self.add_button_part2(new_board,button,category)

    def add_button_part2(self,new_board,button,category):
        custom_buttons=[[]]
        for i in new_board:
        ...
        if category=="Главное меню":
            ...
            self.board[button]=[["Назад"]]
        else:
            custom_buttons.extend(self.admin_buttons["categories"])
        self.board[category]=custom_buttons
        return category

    def delete_buttons(self,button,category):
        try:
            board_spisok=self.board[category]
        except KeyError:
            return "KeyError"
        ...
        for i in board_spisok:
            for b in i:
                if b==button:
                    i.remove(b)
                    self.board.pop(b,None)
        self.board[category]=board_spisok
        return category
    ...
    def name_check(self,name,category):
        board_spisok=self.unpacking(self.board[category])
        admin_spisok=self.unpacking(self.admin_buttons)
